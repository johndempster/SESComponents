...
function NIMX_DeviceExists( DevName : String ) : Boolean ;
...
begin
    ...
    Err := DAQmxResetDevice ( PChar(DevName) ) ;
...
begin
    ...
    Err := DAQmxResetDevice ( PChar(DeviceName) ) ;
    if Err <> 0 then begin
    ...
    NIMX_CheckError(DAQmxGetDevProductType( PChar(DeviceName), CBuf, High(CBuf) )) ;
    FBoardModel := '' ;
    i := 0 ;
    ...
    if (@DAQmxGetDevAIMaxMultiChanRate <> Nil) and
       ...
       Err := DAQmxGetDevAOSampClkSupported( PChar(DeviceName), FDACClockSupported ) ;
       if Err <> 0 then FDACClockSupported := False ;
       if FDACClockSupported then begin
          NIMX_CheckError( DAQmxGetDevAOMaxRate( PChar(DeviceName), DValue )) ;
          FDACMinUpdateInterval := 1.0/DValue ;
          end
       else FDACMinUpdateInterval := 1E-3 ;

       // Determine min/max sampling intervals
       NIMX_CheckError( DAQmxGetDevAIMaxMultiChanRate( PChar(DeviceName), DValue )) ;
       FADCMinSamplingInterval := 1.0/DValue ;
       NIMX_CheckError( DAQmxGetDevAIMinRate( PChar(DeviceName), DValue )) ;
       FADCMaxSamplingInterval := 1.0 /DValue ;

       if DAQmxGetDevDOMaxRate( PChar(DeviceName), DValue ) = 0 then begin
    ...
    // Calibrate device
    //NIMX_CheckError( DAQmxSelfCal( PChar(DeviceName) )) ;
    ...
    if (@DAQmxGetDevAOPhysicalChans <> Nil) and
       ...
       DAQmxGetDevAOVoltageRngs( PChar(DeviceName), AORanges, High(AORanges) ) ;
       FDACMinVolts := 0.0 ;
       FDACMaxVolts := 0.0 ;
...
begin
     ...
     if LibraryHnd <> 0 then begin
       { Get addresses of procedure NIMX_s used }
        ...
        @DAQmxGetDevAIMaxMultiChanRate := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevAIMaxMultiChanRate')) ;
        @DAQmxGetDevAIMinRate := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevAIMinRate'  )) ;
        @DAQmxGetDevAIPhysicalChans := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevAIPhysicalChans'  )) ;
        @DAQmxGetDevAOVoltageRngs := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevAOVoltageRngs'  )) ;
        @DAQmxGetDevAOSampClkSupported := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevAOSampClkSupported'  )) ;
        @DAQmxGetDevAOMaxRate := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevAOMaxRate'  )) ;
        @DAQmxGetDevAOMinRate := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevAOMinRate'  )) ;
        @DAQmxGetDevDOMaxRate := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevDOMaxRate'  )) ;
        @DAQmxGetDevAOPhysicalChans := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevAOPhysicalChans'  )) ;
        @DAQmxGetDevAOVoltageRngs := GetProcAddress(LibraryHnd,PChar('DAQmxGetDevAOVoltageRngs'  )) ;
...
begin
    ...
    for i := 0 to NumVRanges-1 do begin
        ...
        if Err = 0 then begin
          NIMX_CheckError( DAQmxGetAIMax( ADCTask, PChar(ChannelName), DValue ));
...
var
    ...
    SamplingRate : Double ;
    TriggerSource : String ;    // Trigger source terminal for A/D clock
...
//     ChannelList := format( DeviceName + '/AI0:%d', [nChannels-1] ) ;
     ...
     for ch := 0 to nChannels-1 do begin
         ...
         NIMX_CheckError( DAQmxSetAIRngHigh( ADCTaskHandle,
                          PChar(format( DeviceName + '/AI%d',
                          [ADCChannelInputNumber[ch]] )), ADCVoltageRange )) ;
         //outputdebugstring(pchar(format('ch.%d %.4g',[ch,DValue])));
         end ;
...
begin
     case ADCInputMode of
        imDifferential,imBNC2110 : begin
            if ANSIContainsText(FBoardModel,'61') then Result := DAQmx_Val_PseudoDiff
...
begin
    ...
    //outputdebugString(PChar(format('%d ',[NumSamplesRead]))) ;
